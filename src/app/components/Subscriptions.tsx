...
export function Subscriptions({ subscriptions, onAdd, onEdit, onDelete }: SubscriptionsProps) {
  ...
  const filtered = subscriptions.filter((s) => {
    const matchSearch = s.platform.toLowerCase().includes(search.toLowerCase()) || s.plan.toLowerCase().includes(search.toLowerCase()) || s.buyer.toLowerCase().includes(search.toLowerCase());
    const matchCycle = filterCycle === "All" || s.cycle === filterCycle;
  ...
  const downloadCSV = () => {
    const headers = ["Platform", "Plan", "Cost", "Cycle", "Monthly Cost", "Purchase Date", "Expiry Date", "Payment Mode", "Buyer", "Category"];
    const rows = subscriptions.map((s) => [
      s.platform, s.plan, s.cost, s.cycle, getMonthlyCost(s).toFixed(2),
      s.purchaseDate, s.expiryDate, s.paymentMode, s.buyer, s.category,
    ]);
    ...
    <div className="flex flex-col gap-6 p-6 min-h-full" style={{ background: "#f8fafc" }}>
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="flex items-center gap-1 p-1 rounded-xl" style={{ background: "#f1f5f9", border: "1px solid #e2e8f0" }}>
          {["All", "Monthly", "Quarterly", "Annual"].map((cycle) => (
            <button
              key={cycle}
        ...
        <table className="w-full" style={{ borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ borderBottom: "1px solid #f1f5f9", background: "#f8fafc" }}>
              {["Platform", "Plan", "Cost", "Cycle", "Purchase Date", "Expiry", "Payment", "Buyer", "Status", "Actions"].map((h) => (
